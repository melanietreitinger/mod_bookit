...
$DBG = function(string $msg) use ($debug, $dbgprefix) {
    if ($debug) { error_log($dbgprefix . $msg); }
};
...
if (!empty($events)) {
    ...
    foreach ($events as $ev) {
        ...
        if ($eid) { $eventids[] = (int)$eid; }
    }
    $eventids = array_values(array_unique(array_filter($eventids)));

    if ($debug) { $DBG('room augment: have ' . count($eventids) . ' ids'); }
    ...
    if ($eventids) {
        ...
        foreach ($rows as $row) {
            $eid = (int)$row->eventid;
            if (!isset($evRooms[$eid])) { $evRooms[$eid] = ['ids' => [], 'names' => []]; }
            $evRooms[$eid]['ids'][]   = (int)$row->resourceid;
            if (!empty($row->name)) { $evRooms[$eid]['names'][] = (string)$row->name; }
        }

        // Attach to each event (roomids[], roomid, roomname, location)
        foreach ($events as &$ev) {
            $eid = is_array($ev) ? ($ev['id'] ?? null) : ($ev->id ?? null);
            if (!$eid || empty($evRooms[(int)$eid])) { continue; }
            ...
            if (is_array($ev)) {
                ...
                if (empty($ev['location'])) { $ev['location'] = $primaryName; }
            } else { // object
                $ev->roomids   = $ids;
                $ev->roomid    = $ids ? $ids[0] : null;
                $ev->roomname  = $primaryName;
        ...
        if ($debug) { $DBG('room augment: mapped ' . count($evRooms) . ' events'); }
    }
}
...
if ($eventids) {
    ...
    foreach ($events as &$ev) {
        $eid = is_array($ev) ? ($ev['id'] ?? null) : ($ev->id ?? null);
        if (!$eid || !isset($rows2[$eid])) { continue; }
        $dept     = (string)($rows2[$eid]->department ?? '');
        $dbstatus = $rows2[$eid]->bookingstatus;
        
        if (is_array($ev)) {
            if (empty($ev['department']))                 { $ev['department']      = $dept; }
            if (!array_key_exists('bookingstatus', $ev))  { $ev['bookingstatus']    = $dbstatus; }
        } else {
            if (empty($ev->department))                   { $ev->department        = $dept; }
            if (!isset($ev->bookingstatus))               { $ev->bookingstatus     = $dbstatus; }
        }
        
    }
    unset($ev);

    if (!empty($debug)) { $DBG('dept/status augment: mapped '.count($rows2).' rows'); }
}
...
if ($debug && !empty($events)) {
    $one = reset($events);
    $DBG('sample pre-filter: id=' . ($one->id ?? 'null') .
...
$toDT = function($val) {
    ...
    if (is_numeric($val)) {
        ...
        $sec = (int)$val;
        if ($sec > 2000000000) { $sec = (int)round($sec / 1000); }
...
$events = array_filter($events, function ($ev) use ($roomid, $faculty, $status, $search, $debug, $dbgprefix) {
    ...
    if ($roomid) {
        ...
        if (!$hasMatch) {
            if ($debug) {
                error_log($dbgprefix . 'DROP room (by id): want='.(int)$roomid
   ...
   /* --- FACULTY filter (trim + case-insensitive) -------------------- */
    if ($faculty !== '') {
        ...
        if ($evDeptNorm === '') {
            if ($debug) { error_log($dbgprefix . 'DROP faculty: event has empty department'); }
            return false;
        }
        if ($evDeptNorm !== $wantNorm) {
            if ($debug) { error_log($dbgprefix . 'DROP faculty mismatch want=' . $wantNorm . ' ev=' . $evDeptNorm); }
            return false;
        }
    }
...
if ($status > -1) {
    ...
    if ($evStatus === null || $evStatus === '') {
        if ($debug) { error_log($dbgprefix . 'DROP status: event has no bookingstatus'); }
        return false;
    }
    ...
    if ($search !== '') {
        ...
        if (mb_strpos($haystack, $needle) === false) {
            if ($debug) { error_log($dbgprefix . 'DROP search miss'); }
            return false;
        }
    }
